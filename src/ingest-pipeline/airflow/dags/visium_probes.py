...
def get_visium_probe_set_version(data_dir):
    metadata_file_path = data_dir / 'scRNAseq-visium-with-probes-metadata.tsv'
    metadata_df = pd.read_csv(metadata_file_path, sep='\t')
    probe_panel = metadata_df.oligo_probe_panel.iloc[0]
    return 2 if "v2" in probe_panel else 1
...
default_args = {
    ...
    "xcom_push": True,
    "queue": get_queue_resource("visium_probes"),
...
with HMDAG(
    ...
    user_defined_macros={
        ...
        "preserve_scratch": get_preserve_scratch_resource("visium_probes"),
    },
) as dag:
    ...
    def build_cwltool_cmd1(**kwargs):
        ...
        visium_probe_set_version = get_visium_probe_set_version(data_dir)
        command.append("--visium-probe-set-version")
