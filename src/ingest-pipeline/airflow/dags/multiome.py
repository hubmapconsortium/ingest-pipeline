...
def generate_multiome_dag(params: SequencingDagParameters) -> DAG:
    ...
    with HMDAG(params.dag_id,
        ...
        def build_cwltool_cmd1(**kwargs):
            ...
            assay_flags = {'rna':{'10x':'10x_v3', 'snareseq':'snareseq'}, 'atac':{'snareseq':'snareseq'}}

            if assay_type == "10x":
                metadata_file = data_dir / ATAC_METADATA_PATH
                metadata_df = pd.read_csv(metadata_file, sep='\t')
                barcode_offset = metadata_df.barcode_offset.iloc[0]
                if barcode_offset == 0:
                    assay_flags['atac']['10x'] = "snseq"
                elif barcode_offset == 8:
                    assay_flags['atac']['10x'] = "multiome_10x"
...
def get_multiome_dag_params(assay: str) -> SequencingDagParameters:
    ...
    return SequencingDagParameters(
        ...
        assay=assay,
        dataset_type=f"multiome_{assay}",
    )
