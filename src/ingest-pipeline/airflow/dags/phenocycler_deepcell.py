...
with HMDAG(
    ...
    workflow_description = "The Phenocycler pipeline begins with optional masking of user-specified regions defined in an optional geoJSON file describing areas of inclusion or exclusion.  It then breaks the image up into slices that can be reasonably segmented, runs cell and nuclear segmentation on those slices using [DeepCell](https://deepcell.readthedocs.io/en/latest/app-gallery/mesmer.html) with the marker channels specified by the data provider and stitches the segmentation masks back into one mask which can overlay the full expression image.  DeepCell was chosen for segmentation based on its performance in [an evaluation](https://www.molbiolcell.org/doi/full/10.1091/mbc.E22-08-0364) of various methods across multiple tissues using quality metrics that can be calculated without the need for a human-segmented image for comparison.  More information about DeepCell and the associated Mesmer algorithm can be found in the associated publication [here](https://www.nature.com/articles/s41587-021-01094-0).  The segmentation masks and expression images are then used for cell type assignment using RIBCA, Stellar, and DeepCellTypes.  Finally, SPRM is used for spatial analysis of expression data, including computation of various measures of analyte intensity per cell, clustering based on expression and other data, marker computation per cluster, subclustering of cell types assigned by multiple methods and more."
