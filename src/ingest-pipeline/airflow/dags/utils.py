...
from status_change.status_manager import DATASET_STATUS_MAP, StatusChanger, Statuses
...
class PipelineFileMatcher(FileMatcher):
    ...
    ) -> Iterable[Tuple[Pattern, str, str, bool]]:
...
def get_file_metadata(root_dir: str, matcher: FileMatcher) -> List[Mapping[str, Any]]:
    ...
    for dirpth, dirnames, fnames in walk(root_dir):
        ...
        for fn in fnames:
            ...
            add_to_index, description, ontology_term, is_qa_qc = matcher.get_file_metadata(
                relative_path
            )
            if add_to_index:
