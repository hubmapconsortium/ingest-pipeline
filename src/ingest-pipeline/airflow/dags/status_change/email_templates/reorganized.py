...
class ReorganizedStatusEmail(EmailTemplate):
    ...
    def format(self) -> tuple[str, str]:
        subj = f"Datasets successfully created for upload {self.entity_id}!"
        msg = [
    ...
    def dataset_info(self):
        ...
        dataset_info = ["", "Datasets:", "<ul>"]
        for dataset in datasets:
            dataset_info.append(
                f"<li><a href='{get_entity_ingest_url(dataset)}'>{get_entity_id(dataset)}</a>: {dataset.get('status', '').lower()}</li>"
            )
