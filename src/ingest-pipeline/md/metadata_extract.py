...
SCHEMA_BASE_PATH = os.path.join(
    os.path.dirname(os.path.realpath(os.path.dirname(__file__))), "schemata"
)
...
def scan(target_dir, out_fname, schema_fname, yaml_flag=False, component=None):
    ...
    for collection_type in _KNOWN_DATA_COLLECTION_TYPES:
        if collection_type.test_match(target_dir):
            ...
            metadata = collector.filter_metadata(collector.collect_metadata(component, component_process))
            # print('collector: ', repr(collector))
            # print('metadata: %s' % metadata)
            break
    else:
        raise MetadataError("%s does not match any known data collection type" % target_dir)
...
def main(myargv=None):
    ...
    parser = argparse.ArgumentParser(
        description="Scan a directory tree of datafiles and extract metadata"
    )
    parser.add_argument(
        "--out", default=None, help="Full pathname of output JSON (defaults to stdout)"
    )
    parser.add_argument(
        "--schema", default=None, nargs=1,
        help=(
            "Schema against which the output will be checked (default %s)" % default_schema_path
        ),
    )
    ...
    parser.add_argument("--component", default=None,
                        help="Component name to match when building Multi-Assay component metadata",
    )
