...
def close_enough_match(v1, v2, tp):
    ...
    if tp == str:
        ...
        raise MetadataError(
            "close_enough_match does not know how to compare values of type {}".format(tp.__name__)
        )
...
class AkoyaCODEXDataCollection(DataCollection):
    ...
    def find_top(cls, path, target, dir_regex=None):
        ...
        if target in os.listdir(path):
            ...
            if dir_regex is None:
                candidates = [
                    nm for nm in os.listdir(path) if os.path.isdir(os.path.join(path, nm))
                ]
            else:
                candidates = [
                    nm
    ...
    def internal_consistency_checks(self, rslt, experiment_md, test_tpl_lst, channel_names):
        ...
        for rslt_nm, expt_nm, _ in test_tpl_lst:
            try:
                print(
                    ...
                    "exp.json: ",
                    expt_nm,
                    type(experiment_md[expt_nm]),
                    experiment_md[expt_nm],
                )
        ...
        for rslt_nm, expt_nm, tp in test_tpl_lst:
            ...
            if expt_nm not in experiment_md:
                raise MetadataError(
                    "experiment.json is missing expected element {}".format(expt_nm)
                )
            if not close_enough_match(rslt[rslt_nm], experiment_md[expt_nm], tp):
                raise MetadataError(
                    "metadata field {} does not match experiment.json field {}".format(
                        rslt_nm, expt_nm
                    )
                )
        ...
        if channel_names is not None:
            arr = [
                elt
                for elt in channel_names
                if elt.lower() not in ["blank", "empty"]
                and not elt.startswith("DAPI")
                and not elt.startswith("HOECHST")
            ]
            if not close_enough_match(rslt["number_of_antibodies"], len(arr), int):
                raise MetadataError(
                    "metadata field number_of_antibodies does not match length of"
    ...
    def filter_metadata(self, raw_metadata):
        ...
        tests = [
            ("resolution_x_value", "xyResolution", float),
        ...
        if (
            "channelNames" not in experiment_md
            or "channelNamesArray" not in experiment_md["channelNames"]
        ):
        ...
        # self.internal_consistency_checks(rslt, experiment_md, tests,
        # experiment_md['channelNames']['channelNamesArray'])
